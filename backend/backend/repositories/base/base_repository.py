...
class BaseRepository(ABC):
    ...
    async def get_by(self, model_type: type, attr: str, value: Any, one: bool = False) -> Union[Optional[Any], List[Any]]:
    ...
    async def get_entity_with_max_column_value(self, model_type: type, filter_data: dict, column_name: str) -> Optional[Any]:
        column = getattr(model_type, column_name, None)
        if column is None:
            raise ValueError(f"O atributo '{column_name}' não existe no modelo.")
        base_query = self.__get_filter_query(filter_data=filter_data, model_type=model_type)
    ...
    def __query_for_interval(self, intervals: List[Tuple], query: Select[Tuple], model_type: type) -> Select[Tuple]:
    ...
    def __query_for_point(self, query: Select[Tuple], column: Any, start_value: List[Any], end_value: List[Any]) -> Select[Tuple]:
        return query.filter(column >= start_value, column <= end_value)

    def __query_for_start_interval(self, query: Select[Tuple], column: Any, start_value: Any) -> Select[Tuple]:
        return query.filter(column >= start_value)

    def __query_for_end_interval(self, query: Select[Tuple], column: Any, start_value: Any) -> Select[Tuple]:
        return query.filter(column <= start_value)

    def __query_for_common_attributes(self, common_attributes: List[Tuple], query: Select[Tuple], model_type: type) -> Select[Tuple]:
    ...
    def __get_filter_query(self, filter_data: dict, model_type: type, query: Select[Tuple] = None) -> Select[Tuple]:
        query = select(model_type) if query is None else query
        items = filter_data.items()
        intervals = [(attr, value) for attr, value in items if str(attr).endswith("__start") or str(attr).endswith("__end")]
    ...
    async def filter_and_get_first(self, filter_data: dict, model: type) -> Optional[Any]:
        entities: List[Any] = await self.filter(filter_data=filter_data, model_type=model)
        return entities[0] if entities else None

    async def filter_records(self, filter_data: dict, model_type: type, return_keys: bool = True) -> Any:
        query: Select[Tuple] = Select(*model_type.__table__.columns)
        query = self.__get_filter_query(filter_data=filter_data, model_type=model_type, query=query)
        result = await self.db.execute(query)
        records = result.all()
        if not return_keys:
            return records
    ...
    async def filter_columns(self, model_type: type, filter_data: dict, columns: List[str]) -> List[Any]:
